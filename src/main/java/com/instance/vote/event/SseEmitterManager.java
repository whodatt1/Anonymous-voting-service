...
public class SseEmitterManager {
    ...
    // 교체 방식으로 변경 pollId당 emitter를 항상 1개로 유지
...
//        }
        emitters.remove(pollId, emitter);
    }
    ...
    public void broadcast(Long pollId, Object data) {
        //SseEmitter emitterList = emitters.get(pollId);
        SseEmitter emitter = emitters.get(pollId);
...
//        list.forEach(SseEmitter::complete);
        ...
        SseEmitter emitter = emitters.remove(pollId);
        if (emitter == null) return;
        emitter.complete();
    }

    // 브라우저 탭을 강제로 닫았을때도 SSE 연결이 유지됨
