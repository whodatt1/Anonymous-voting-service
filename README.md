...
이 프로젝트는 **Claude Code(Anthropic)와 AI 페어 프로그래밍 방식으로 개발**했습니다.

단순히 코드 생성을 맡긴 것이 아니라, 아래 3단계 워크플로우를 강제해 AI를 활용했습니다.

1. **설계 토론**: 새 기술·패턴 도입 시 AI에게 도입 배경·대안·트레이드오프를 먼저 요구하고 토론
2. **의사결정 문서화**: 토론이 정리되면 [`docs/decisions.md`](docs/decisions.md)에 결론과 근거를 압축 기록 (현재 59건 누적)
3. **개발자 주도 구현**: 코드는 직접 작성. AI는 리뷰어·리팩토링 조언자 역할만 수행

AI가 생성한 코드를 그대로 붙여넣는 방식과 달리, 모든 기술 선택의 이유를 본인의 언어로 설명할 수 있는 상태를 목표로 했습니다.
...
| `GET /api/votes/{shareCode}/host` | 분당 10회 |
| `POST /api/votes` | 분당 5회 |
...
전체 타임라인 하단 패널에서 `tomcat_connections_current_connections` 지표를 함께 관찰했습니다. SSE 활성 커넥션 수와 NIO 커넥션 수가 동일 궤적으로 움직이며, **SSE 1개가 NIO 커넥션 1개를 점유**하는 구조가 실측으로 확인됐습니다. Phase 2 전환 기준("NIO 커넥션이 8192에 근접하는 시점")을 Grafana에서 직접 관찰할 수 있는 체계가 마련됐습니다.
...
| 조건 | 현재 스레드 수 | 사용 중인 스레드 |
|---|---|---|
| 150개 동시 종료 (1차 실험) | ~75 (집중 스파이크) | 0 유지 |
| 점진적 ramp-down (2차 실험) | 스파이크 발생 (분산) | 0 유지 |

점진적 종료에서도 스파이크가 발생했습니다. **스파이크는 단순 동시 종료 아티팩트가 아니라, Tomcat이 SSE 연결 종료 콜백(`onCompletion`/`onError`) 처리 시 스레드 풀을 일시적으로 사용하는 실제 동작**입니다. 동시 종료 시 집중 발생, 점진 종료 시 분산 발생하는 패턴 차이가 있을 뿐입니다. 다만 사용 중인 스레드(실제 요청 처리)는 어떤 구간에서도 영향을 받지 않았습니다.

### k6 결과 — 35,718건 전량 성공, p95 = 30.01ms

![k6 metrics](docs/images/load-test/load-test-k6-metrics.png)

| 지표 | 값 | 설명 |
|---|---|---|
| http_req_failed | **0.00%** (35,718건) | 에러 없음 |
| p(90) | **27.18ms** | 전체 요청의 90%가 이 시간 이하로 응답 |
| p(95) | **30.01ms** | 전체 요청의 95%가 이 시간 이하로 응답 |
| max | 2m49s | sseScenario의 `http.get()`이 SSE 연결을 유지한 시간이 포함된 값 — 평균(avg)을 왜곡하므로 지표로 사용하지 않음 |
